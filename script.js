...
// Lightweight music behavior:
// 1) Try autoplay (works only when the browser permits it).
// 2) If blocked, the very first tap/click anywhere starts the song.
// This is the closest browsers allow to "langsung play" with sound.
async function tryInitialAutoplay() {
  try {
    song.volume = 0.42;
    await song.play();
    musicStarted = true;
    ...
    // Expected on Chrome/Safari mobile until a user gesture happens.
  }
}
...
window.addEventListener("DOMContentLoaded", tryInitialAutoplay, { once: true });
document.addEventListener("pointerdown", startMusicOnFirstGesture, { once: true, passive: true });
